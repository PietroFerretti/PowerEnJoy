...
The definition of the acronyms for the scale factors and the cost drivers can be found in their respective sections.
...
Function points are useful for expressing the number of business functionalities the software has to provide to a user and are used to compute an estimation of its size.
...
Internal Logic Files are all kinds of data used and managed by the application in  order to offer the expected functions.\\
Data will be organized in the following tables in the DB:

\begin{itemize}

	\item \textbf{User} : name, surname, username, password, date of birth, email, licenseID, cvv, cardNumber, accountStatus
...
\end{itemize} The software will operate directly on the single rows and tables, joining them appropriately if needed.\\
...
\begin{itemize}
	...
	\item \textbf{Eucaris API}:
		...
		\end{itemize}
\end{itemize} In the final analysis, as the data involved are only strings and numbers of small size, we can assess these logic files as low complexity.\\
...
\begin{center}
	\begin{tabular}{|p{5cm}|p{1cm}|}
		...
		\hline
	\end{tabular}
\end{center} This table contains a recap of the evaluations of each type of function point.\\
