...
\begin{description}
  ...
  \item[{[G21]}]{Users must be able to search a PGS.}
\end{description}
...
\item[{[G13]}]{Users must be able to search a PGS.}
\begin{itemize}
\end{itemize}
}

\item[{[G14]}]{The stations proposed by the system with the money saving option must be chosen in a way that ensures a uniform distribution of cars in the city.
\begin{itemize}
...
\item[{[G15]}]{The system must apply a discount of 10\% on the last ride if the user took at least two other passengers onto the car.
\begin{itemize}
	\item{[RE.7.1] The system must keep track until the end of the reservation of the amount the user has to pay.}
...
\item[{[G16]}]{The system must apply a discount of 20\% on the last ride if the car is left with more than 50\% of remaining battery charge.
\begin{itemize}
	\item{[RE.7.1] The system must keep track until the end of the reservation of the amount the user has to pay.}
...
\item[{[G17]}]{The system must apply a discount of 30\% on the last ride if the car is left at special parking areas where they can be recharged, and the user takes care of plugging the car into the power grid.
\begin{itemize}
	\item{[RE.7.1] The system must keep track until the end of the reservation of the amount the user has to pay.}
...
\item[{[G18]}]{The system must charge 30\% more if the car is left at more than 3 km from the nearest power grid station or with less than 20\% remaining battery charge.
\begin{itemize}
	\item{[RE.7.1] The system must keep track until the end of the reservation of the amount the user has to pay.}
...
\item[{[G19]}]{Users with outstanding bills cannot have access to the cars and reservations until the bills are paid.
\begin{itemize}
...
\item[{[G20]}]{Users with outstanding bills should have a way to pay them.
\begin{itemize}
...
\item[{[G21]}]{Available cars should actually be available to the user and in good conditions (i.e. not dirty, damaged, and/or with low battery)
\begin{itemize}
...
We identified the following use cases for our software:
    ...
    \item{\textbf{[UC.7]} A user searches a PGS.}
		...
		\item{\textbf{[UC.13]} A user pays a pending bill.}
...
Figure \ref{use_case_diagram} shows the relationships between use cases and involved actors.
  ...
  \paragraph{[UC.7] A user searches a PGS.}	~\\
	\begin{tabularx}{\textwidth}{  l  X  }
		\hline
		Actor & User\\
		\hline
		Goal & [G21]\\
		\hline
		Preconditions & The user is driving and he didn't choose MSO.\\
		\hline
		Execution Flow & \begin{enumerate}
			\item{The user presses on the "find charging station" button on the display of the car.}
			\item{The user inserts the destination.}
			\item{The system checks in the DB the PGS available.}
			\item{The system shows to the user the list of PGS in order of closeness.}
			\item{By pressing on a PGS shown by the display, the user can set the chosen PGS as destination.}
		\end{enumerate}\\
		\hline
		Postconditions & The user knows the PGSs near his destination.\\
		\hline
		Exceptions & \begin{enumerate}
			\item{There aren't any PGSs available
		\end{enumerate}\\
		\hline
	\end{tabularx}
...
\begin{itemize}
  ...
  \item{\textit{User Friendliness}:
    The user interface has to be as simple and intuitive as possible because
  ...
  \item{\textit{Security}:
    \begin{itemize}
      ...
      \item{sensitive data like usernames, driving license IDs and so on will be saved encrypted on the database, and passwords will be hashed using the SHA-256 algorithm.}
\end{itemize}}
  ...
  \item{\textit{Portability}: in the matter of server-side, we will implement our software in order to be executed on the most commercial server;
                              ...
                              while the web app will be compatible with the most used browser as Chrome, Firefox, Opera, Safari, etc\ldots}
