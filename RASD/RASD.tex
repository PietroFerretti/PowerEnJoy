...
idee:
i database e tutto sono su dei server centrali
c'è un sistema sulla macchina che controlla tutti i sensori, controlla l'applicazione a schermo e comunica con i server centrali
l'applicazione web comunica con i server centrali per avere accesso a dati/database
l'applicazione mobile pure
...
users are people who registered to our system
users can drive (they have a valid driving license)
users want to get to a place to another
users prefer to have access to ready available cars
users 
...
I COSTRAINTS SONO LIMITI AL SOFTWARE PER REGOLAMENTAZIONI, LIMITAZIONI DELL'HARDWARE ECC.

interfacce con altre applicazioni
operazioni in parallelo
funzioni di controllo
protocolli di handshake (why)
requisiti di reliability
criticalità
CONSIDERAZIONI SULLA SICUREZZA FISICA (safety) DEGLI UTENTI

come è fatto l'hardware lo spostiamo in assumptions?
...
Our software will offer to the users a wide choice (2 of them) of interfaces:
A mobile application
A web application

Additionally an application on the car's display when the user is in the car

This is what they will look like:
the users will be able to perform all the functions listed in section \ref{goals} 

the interfaces will be user-friendly and well designed
every button's function must be clear and evident

every function will be reachable in a maximum of 3 clicks/presses/taps
