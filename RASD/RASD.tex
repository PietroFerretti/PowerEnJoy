...
\emph{\textbf{Requirements Analysis and Specification Document}}}
...
\item{\textit{Power grid station} or \textit{Charging station}: also called charging station. A place where cars can be parked and plugged in. While a car is plugged in a power grid station its battery will be recharged. Power grid stations are by definition safe areas.}
...
\begin{figure}[H]
	\centering
	\makebox[\textwidth][c]{
		\includegraphics[width=450pt]{ClassDiagram.png}
	}
...
\begin{figure}[H]
	...
	\begin{minipage}{.5\textwidth}
		\centering
		\includegraphics[width=100pt]{mobile/Reservations.png}
		\caption{List of user's reservations}
	\end{minipage}
\end{figure}
...
\begin{itemize}
  \item{\textit{Payment APIs}:
    ...
    \item{\textit{Driving License Validation API}:
      ...
      \end{itemize}}
      \item{\textit{Sign Up APIs}:
      	\begin{itemize}
      		\item{\textit{Facebook API}:
      			\href{https://developers.facebook.com/docs/facebook-login/}{\underline{Login with Facebook}}}
      		\item{\textit{Google API}:
      			\href{https://developers.google.com/identity/}{\underline{Google Identity Platform}}}
      	\end{itemize}
      \item{\textit{Mapping/Routing API}:
        \begin{itemize}
          \item{\textit{GraphHopper API}:
            \href{https://graphhopper.com}{\underline{GraphHopper}}}
        \end{itemize}}
    }
\end{itemize}
...
\item[{[G5]}]{Users must be able to cancel a reservation if they decide to not actually use the car.}
\begin{itemize}
	\item{[RE.5.1] If the user has reserved a car the system must offer an interface to cancel the reservation.}
	\item{[RE.5.2] When the user selects to cancel the reservation, the system must free the car and allow the user to reserve another car.}
\end{itemize}
...
\begin{itemize}
	\item{[RE.7.1] The system must keep track until the end of the reservation of the amount the user has to pay.}
	\item{[RE.9.1] The moment a car is parked in a safe area, the system must ask through an interface on the car screen if the user wants to end the reservation or just park temporarily.}
	\item{[RE.10.1] When a car is temporarily parked and locked, the system keeps on charging the user but with a different rate than that used in RE.7.3.}
	\item{[RE.10.2] After a temporarily parked car is unlocked by the user and the engine is turned on, the system starts charging the user with the rate used in RE.7.3 again.}
\end{itemize}
}
...
\item[{[G12]}]{The system must provide a money saving option to the user, proposing a suitable  station near the user’s final destination. The user will get a discount if they park the car there and plug it in the power grid.
\begin{itemize}
...
\item[{[G21]}]{Users must be able to easily find power grid stations and safe areas.
	\begin{itemize}
		\item{[RE.21.1] Users must have a way to search for power grid stations, for example with an option on the car navigator.}
		\item{[RE.21.2] Users must have a way to search for safe areas, for example with an option on the car navigator.}
	\end{itemize}}
...
We identified the following use cases for our software:
	\begin{itemize}
	    ...
	    \item{\textbf{[UC.7]} A user searches for a PGS.}
		\item{\textbf{[UC.8]} A user ends a ride.}
		\item{\textbf{[UC.9]} A user parks the car without ending the ride.}
		\item{\textbf{[UC.10]} A user ends a ride while the car is temporarily parked.}
		\item{\textbf{[UC.11]} The system suggests to the user a PGS to park the car at and save money.}
		\item{\textbf{[UC.12]} Payment}
		\item{\textbf{[UC.13]} A user pays an outstanding bill.}
		\item{\textbf{[UC.14]} A user reports a problem to the system.}
	\end{itemize}
...
Figure \ref{use_case_diagram} shows the relationships between use cases and involved actors.
	...
	\begin{figure}
		...
		\makebox[\textwidth][c]{
			...
			\input{images/use_case_diag2.pdf_tex}
		}
	...
	\begin{tabularx}{\textwidth}{  l  X  }
		...
		Exceptions & \begin{enumerate}
			...
			\item{The user is suspended by the system. In this case, the application doesn't let them reserve the car and reminds them that they have to pay the outstanding bill(s).}
		\end{enumerate}\\
		\hline
	\end{tabularx}
  ...
  \paragraph{[UC.7] A user searches for a PGS.}	~\\
	\begin{tabularx}{\textwidth}{  l  X  }
		\hline
		Actor & User\\
		\hline
		...
		\hline
		Execution Flow & \begin{enumerate}
			\item{The user presses on the "find charging stations" button on the display of the car.}
			\item{The user inserts the destination.}
			\item{The system looks in the DB for available PGSs.}
			\item{The system shows to the user the list of PGS in order of distance.}
			\item{By pressing on a PGS shown by the display, the user sets the chosen PGS as destination.}
		\end{enumerate}\\
		\hline
		Postconditions & The user knows the PGSs near his destination and has set one as destination.\\
		\hline
		Exceptions & \begin{enumerate}
			\item{There aren't any available PGSs, the user can try again with a different destination}
		\end{enumerate}\\
		\hline
	\end{tabularx}
	...
	\begin{figure}[H]
		...
		\makebox[\textwidth][c]{
			\def\svgwidth{450pt}
			\input{images/search_pgs.pdf_tex}
		}
	\end{figure}

	\paragraph{[UC.8] A user ends a ride.}	~\\
	\begin{tabularx}{\textwidth}{  l  X  }
		\hline
		Actor & User\\
		\hline
		...
		\paragraph{[UC.9] A user parks the car without ending the ride.}	~\\
		\begin{tabularx}{\textwidth}{  l  X  }
			\hline
			Actor & User\\
			\hline
			...
			\paragraph{[UC.10] A user ends a ride while the car is temporarily parked.}	~\\
			\begin{tabularx}{\textwidth}{  l  X  }
				\hline
				Actor & User\\
				\hline
			...
			\newpage
			\paragraph{[UC.11] The system suggests to the user a PGS to park the car at and save money.}	~\\
			\begin{tabularx}{\textwidth}{  l  X  }
				\hline
				Actor & System, User\\
				\hline
			...
			\begin{tabularx}{\textwidth}{  l  X  }
				...
				Exceptions & \begin{enumerate}
					...
					\item{The transaction is rejected. In this case, the system suspends the user until he pays the outstanding bill.}
				\end{enumerate}\\
				\hline
			\end{tabularx}
			...
			\paragraph{[UC.13] A user pays an outstanding bill.}	~\\
			\begin{tabularx}{\textwidth}{  l  X  }
				\hline
				...
				Execution Flow & \begin{enumerate}
					...
					\item{The user selects the outstanding bill.}
			...
			\paragraph{[UC.14] A user reports a problem to the system.}	~\\
			\begin{tabularx}{\textwidth}{  l  X  }
				\hline
				Actor & User, System\\
				\hline
...
\begin{itemize}
  ...
  \item{\textit{User Friendliness}:
    The user interface has to be as simple and intuitive as possible as the application will also be used by users that could feel uncomfortable with the technology.}

  \item{\textit{Reliability}: we assume that our software will give the right result for at least 99\% of the requests.}

  \item{\textit{Availability}: we assume that our software will be available with a rate of 99\%.
                               In order to ensure that rate, patches will deployed during the night and time of low traffic.}

  \item{\textit{Security}:
    \begin{itemize}
      \item{every message exchanged between the system and the user will be protected by using the TLS protocol.}
      \item{sensitive data like names, driving license IDs and so on will be encrypted on the database, and passwords will be hashed using the SHA-256 algorithm.}
\end{itemize}}
  \item{\textit{Maintainability}: the software must be accompanied by a complete documentation to let other developers correct, improve the code or integrate new functions more easily.}

  \item{\textit{Portability}: server-side, the software must be developed so that it can be executed on most commercial servers;
                              client-side the software must be compatible with iOS, Android and Windows Phone regarding the mobile application, while the web app must be compatible with at least the most used browser, i.e. Chrome, Firefox, Opera, Safari, Internet Explorer.}
...
\lstinputlisting{powerenjoy_alloy.als}
...
\begin{itemize}
  \item{Pietro Ferretti: 45 hours}
  \item{Nicole Gervasoni: 45 hours}
  \item{Danilo Labanca: 45 hours}
\end{itemize}
\newpage{}
...
\begin{itemize}
  \item{RASD v1.0, published on November 13, 2016}
  \item{RASD v1.1, published on December 11, 2016
  	\begin{itemize}
  		\item{Added goal G21}
  		\item{Added use case UC.7}
  		\item{Added GraphHopper API}
  		\item{Improved english grammar}
  	\end{itemize}}
  	\item{RASD v1.2, published on January 8, 2017
  	\begin{itemize}
  		\item{Updated mockups pictures according to the UX in the DD v1.0}
  	\end{itemize}}
\end{itemize}
\end{document}
